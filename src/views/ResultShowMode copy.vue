    ...
    for (const stockCode in rawData) {
      if (rawData.hasOwnProperty(stockCode)) {
        ...
        intervals.forEach((interval, index) => {
            ...
            dataObjects.value[index].startDate = interval.start;
            dataObjects.value[index].endDate = interval.end;
            dataObjects.value[index].stockFrom = '系统模式';
            ...
            dataObjects.value[index].imagePath = imagePath;
        });
        dataObjects2.value[0].searchCount = intervals.length; // 更新相似结果个数
        dataObjects2.value[0].searchType = '多股票横向查找'; // 更新查找类型
        dataObjects2.value[0].futureTime = '10'; // 默认未来三天
